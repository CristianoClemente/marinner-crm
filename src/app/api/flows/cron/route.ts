 ...
 *
 * Auth: `AUTOMATION_CRON_SECRET` / `CRON_SECRET` via `x-cron-secret` or
 * `Authorization: Bearer` (Vercel Cron).
 *
 * Hosting: Vercel Cron (`/api/cron/daily`) or hit this URL on a schedule.
 * A 5-minute interval is more than enough for a 24h timeout default;
 * once per hour would also be acceptable for low-volume tenants.
 */
