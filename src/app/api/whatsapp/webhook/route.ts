...
async function handleStatusUpdate(status: {
  ...
  await applyMessageStatusUpdate(supabaseAdmin(), {
    providerMessageId: status.id,
    status: status.status,
    timestampMs: parseInt(status.timestamp, 10) * 1000,
  })
}
...
async function processMessage(
  ...
  // Reactions short-circuit â€” they aren't messages. Resolve contact /
  // conversation only for reaction persistence.
  ...
  const contentType =
    message.type === 'sticker'
      ...
      : message.type === 'interactive'
        ? 'interactive'
        : ['text', 'image', 'document', 'audio', 'video', 'location', 'template'].includes(
              message.type
            )
          ? message.type
          : 'text'

  await processNormalizedInboundMessage(supabaseAdmin(), {
    accountId,
    configOwnerUserId,
