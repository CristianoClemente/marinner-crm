...
export function validateInteractivePayload(
  ...
  if (typeof p.body !== 'string' || p.body.trim() === '') {
    return fail('O corpo da mensagem interativa é obrigatório.')
  }
  if (p.body.length > INTERACTIVE_LIMITS.bodyMaxLength) {
    return fail(
      `O corpo do texto excede o limite de ${INTERACTIVE_LIMITS.bodyMaxLength} caracteres.`,
    )
  }
  ...
  if (p.kind === 'buttons') {
    ...
    if (buttons.length > INTERACTIVE_LIMITS.maxButtons) {
      return fail(
        `Uma mensagem com botões permite no máximo ${INTERACTIVE_LIMITS.maxButtons} botões.`,
      )
    }
  ...
  if (p.kind === 'list') {
    ...
    for (const section of list.sections) {
      if (!section || !Array.isArray(section.rows)) {
        return fail('Toda seção da lista precisa de linhas.')
      }
      for (const row of section.rows) {
        total++
        if (!row || typeof row.id !== 'string' || row.id.trim() === '') {
          return fail('Toda linha da lista precisa de um id.')
        }
        if (seen.has(row.id)) {
        ...
        if (typeof row.title !== 'string' || row.title.trim() === '') {
          return fail('Toda linha da lista precisa de um título.')
        }
        if (row.title.length > INTERACTIVE_LIMITS.listRowTitleMaxLength) {
          return fail(
            `O título da linha "${row.title}" excede o limite de ${INTERACTIVE_LIMITS.listRowTitleMaxLength} caracteres.`,
          )
        }
        if (
          row.description &&
          ...
          return fail(
            `A descrição da linha da lista excede o limite de ${INTERACTIVE_LIMITS.listRowDescriptionMaxLength} caracteres.`,
          )
        }
      }
    }
  ...
  return fail('A mensagem interativa deve ser botões de resposta ou uma lista.')
}
