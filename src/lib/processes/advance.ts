...
export function isAdvanceMode(value: unknown): value is AdvanceMode {
  return value === "free" || value === "sequential";
}

/**
 * Resolve o próximo estágio ao avançar.
 * - sequential (default): próxima posição; allow_skip/force para pular adiante.
 * - free + target: qualquer etapa distinta do template (inclui voltar).
 * - free sem target: comporta-se como próxima sequencial (botão Avançar).
 */
export function resolveAdvance(input: {
  ...
  const mode: AdvanceMode = input.advanceMode ?? "sequential";

  if (mode === "free" && input.targetStageId) {
    ...
    if (current && target.id === current.id) {
      return { ok: false, message: "O card já está nesta etapa." };
    }
