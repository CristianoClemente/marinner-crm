...
export function validateStepsForActivation(steps: StepLike[]): ValidationIssue[] {
  ...
  if (!Array.isArray(steps) || steps.length === 0) {
    issues.push({
      path: 'steps',
      message: 'automações ativas precisam de pelo menos um passo',
    })
    return issues
  }
  walk(steps, '', issues)
...
function validateOne(step: StepLike, path: string, issues: ValidationIssue[]): void {
  ...
  switch (step.step_type) {
    ...
    case 'close_conversation':
      ...
      break
    default:
      issues.push({ path, message: `tipo de passo desconhecido: ${step.step_type}` })
  }
}
...
export function validateTriggerForActivation(
  ...
  if (triggerType === 'keyword_match') {
    ...
    if (!Array.isArray(ids) || ids.length === 0) {
      ...
      issues.push({
        path: 'trigger.reply_ids',
        message: 'os ids de resposta não podem ser vazios',
      })
    }
  }
