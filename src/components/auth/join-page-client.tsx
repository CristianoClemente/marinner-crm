...
const FAIL_COPY: Record<PeekFail["reason"], { title: string; body: string }> = {
  not_found: {
    ...
    body: "Este link não corresponde a um convite válido. Confira a URL ou peça à pessoa que convidou para enviar um novo.",
  },
  used: {
...
export function JoinPageClient({ brand }: { brand?: AuthBrand | null }) {
  ...
  const brandLogo = brand?.logoUrl || DEFAULT_LOGO_SRC;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error ||
              "Você já está em outra conta. Entre com um e-mail diferente para ingressar nesta.",
          );
        } else {
  ...
  const inviteHeader = (
    <CardHeader className="items-center text-center">
      <div className="mb-2 flex size-12 items-center justify-center overflow-hidden rounded-xl bg-primary/10 p-1.5 ring-1 ring-primary/20">
        {brand ? (
      ...
      {brand ? (
        <p className="mb-1 text-sm font-medium text-foreground">{brand.name}</p>
      ) : null}
