...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'nossa conta Marinner',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // para contextos multi-equipe em que "nossa conta Marinner"
    // não bastaria para desambiguar.
    const accountName = result?.accountName ?? 'nossa conta Marinner';
        ...
        {result ? (
          <>
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="rounded-md border border-amber-500/50 bg-amber-500/15 px-3 py-2 text-sm text-amber-200">
              ...
              {/* Anchor styled with `buttonVariants` rather than wrapping
                  in <Button asChild>. The marinner Button is the Base UI
