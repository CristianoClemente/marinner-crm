...
marinner ships a [Model Context Protocol](https://modelcontextprotocol.io)
...
[`marinner-mcp`](https://www.npmjs.com/package/marinner-mcp). Under the hood
...
data or send messages, add `"MARINNER_ENABLE_WRITES": "true"` (and
`"MARINNER_ENABLE_BROADCASTS": "true"` for mass sends) to `env`.
