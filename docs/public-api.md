...
The public API lets you drive your marinner instance from your own
...
```
Authorization: Bearer marinner_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** marinner
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer marinner_live_xxx"
```
...
Rather than polling, register an endpoint and marinner will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; marinner keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer marinner_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/marinner", "events": ["message.received"] }'
...
Headers: `X-Marinner-Event`, `X-Marinner-Webhook-Id`, and `X-Marinner-Signature`.

### Verifying the signature

`X-Marinner-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages marinner stores (inbox + API sends), not broadcast-only
...
[#245](https://github.com/CristianoClemente/marinner-crm/issues/245). Future ideas
