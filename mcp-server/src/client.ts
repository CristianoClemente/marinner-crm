// ============================================================
// marinner public API client.
//
// A thin wrapper over the `/api/v1` REST surface. It attaches the
// bearer key, unwraps the `{ data }` / `{ error }` envelope, and
// turns API failures into a typed MarinnerApiError the tools can render
// cleanly. Nothing here knows about MCP — it's just the CRM API.
// ============================================================

import type { Config } from './config.js';

/** A structured error from the marinner API envelope (`{ error: { code, message } }`). */
export class MarinnerApiError extends Error {
  readonly status: number;
  readonly code: string;

  constructor(status: number, code: string, message: string) {
    super(message);
    this.name = 'MarinnerApiError';
    this.status = status;
    this.code = code;
  }
}
...
export class MarinnerClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      throw new MarinnerApiError(
        0,
        'network_error',
        `Could not reach marinner at ${this.baseUrl}: ${(err as Error).message}`,
      );
    }
    ...
    if (text) {
      try {
        ...
        if (!res.ok) {
          throw new MarinnerApiError(res.status, 'internal', text.slice(0, 500));
        }
      }
    }
    ...
    if (!res.ok) {
      ...
      throw new MarinnerApiError(res.status, code, message);
    }
