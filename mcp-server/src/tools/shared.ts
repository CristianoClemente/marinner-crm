...
// layer dumb). On a MarinnerApiError we return an `isError` result with
...
import { MarinnerApiError } from '../client.js';
...
/**
 * Wrap a tool handler so any MarinnerApiError becomes a clean, model-
...
export function handle<A>(
  ...
  return async (args: A) => {
    try {
      ...
      if (err instanceof MarinnerApiError) {
        return errorResult(`marinner API error [${err.code}]: ${err.message}`);
      }
