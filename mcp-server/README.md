...
**[marinner](https://github.com/CristianoClemente/marinner-crm)** — the self-hostable
...
It's a thin wrapper over marinner's public [`/api/v1`](../docs/public-api.md)
REST API. All auth, scoping, and rate limiting are enforced by your
marinner instance — this server just exposes the API as MCP tools.

## Prerequisites

1. A running marinner instance (your own self-hosted deploy).
...
| ------------------------- | -------- | -------------------------------------------------------------- |
| `MARINNER_BASE_URL`          | yes      | Your instance URL, e.g. `https://crm.example.com`              |
| `MARINNER_API_KEY`           | yes      | An API key from the dashboard                                  |
| `MARINNER_ENABLE_WRITES`     | no       | `true` to expose contact writes + message sending             |
| `MARINNER_ENABLE_BROADCASTS` | no       | `true` to expose mass broadcasts (needs `MARINNER_ENABLE_WRITES`) |
...
```jsonc
{
  "mcpServers": {
    "marinner": {
      ...
      "args": ["-y", "marinner-mcp"],
      "env": {
        "MARINNER_BASE_URL": "https://crm.example.com",
        "MARINNER_API_KEY": "marinner_live_xxxxxxxxxxxxxxxxxxxxxxxx"
      }
    }
  }
}
...
"env": {
  "MARINNER_BASE_URL": "https://crm.example.com",
  "MARINNER_API_KEY": "marinner_live_xxxxxxxxxxxxxxxxxxxxxxxx",
  "MARINNER_ENABLE_WRITES": "true",
  "MARINNER_ENABLE_BROADCASTS": "true"
}
```
...
1. **Read-only by default.** Write and broadcast tools are not even
   ...
   `MARINNER_ENABLE_WRITES` / `MARINNER_ENABLE_BROADCASTS`.
2. **API-key scopes.** Whatever the guards allow, your marinner instance
